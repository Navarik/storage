...
describe('Entity versioning', () => {
  before(() => storage.init().then(() => storage.create('profile.user', fixtures[0])).then(entity => {
      id = entity.id
    })
  )
...
describe('Entity versioning disabled (trackVersions=false)', () => {
  const storageNoVer = createStorage({
    schema: fixtureSchemata,
    trackVersions: false
  })
  const { canCreate, cannotCreate, cannotUpdate, canUpdate } = createSteps(storageNoVer)

  before(() => storageNoVer.init().then(() => storageNoVer.create('profile.user', fixtures[0])).then(entity => {
      id = entity.id
    })
  )
  ...
  it('only the latest version is directly available', async () => {
    ...
    response = await storageNoVer.get(id)
    expectEntity(response)
    expect(response.body).to.eql(lastVersion)
    expect(response.version).to.eql(fixtures.length)

    response = await storageNoVer.find(lastVersion)
    expect(response).to.be.an('array')
    expect(response).to.have.length(1)
    expectEntity(response[0])
    expect(response[0].body).to.eql(lastVersion)
